...
const filteredNasaData = computed(() => {
    if (selectedCameraChips.value?.length === 0) return nasaData.value.entries
...
onMounted(() => {
    if (nasaData.value && getCameraQuery.value) {
        // Built array from camera query params
        const selectedCameras = getCameraQuery.value.split(',').map(camera => camera.trim());
        ...
        selectedCameraChipIndexes.value = preSelectedIndexes.filter(index => index !== -1); // Remove -1 indexes (not found)
    }
});
...
watch(selectedCameraChipIndexes, (newValue) => {
    if (newValue) {
        getCameraQuery.value = newValue.map(index => nasaData.value.cameras[index].name).join(',');
    }
});
</script>
        ...
        <v-row class="d-flex justify-center mb-8">
          <v-chip-group multiple v-model="selectedCameraChipIndexes">
                ...
                >
                    ...
                    <v-icon v-if="camera.total_photos === 0" icon="mdi mdi-cctv-off" class="mr-2"></v-icon>
                    <v-icon v-else icon="mdi mdi-cctv" class="mr-2"></v-icon>
        ...
        <v-row v-if="filteredNasaData.length > 0">
            <transition-group name="fade">
               ...
               <v-card :to="`${menu.mars.link}/${item.index}`" class="h-100">
                ...
                <p class="mb-4">
                    <v-icon icon="mdi mdi-cctv"></v-icon>
