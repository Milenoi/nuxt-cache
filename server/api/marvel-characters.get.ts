...
export default defineEventHandler(async (event) => {
    ...
    const cacheKeyMarvel = isDetailPage ? `marvel-detail-${hash}` : `marvel-list-${hash}`;
    const cachedMarvelData = await useStorage('redis').getItem(cacheKeyMarvel);
    ...
    if (cachedMarvelData) {
        ...
        const fetchPromises = idsArray.map(async (id: string) => {
            try {
                ...
                const response: any = await $fetch(getMarvelApi(id));
        ...
        const mergedResults = allData.reduce((accumulator: any[], currentValue, index) => {
            ...
            if (currentValue.data && currentValue.data.results) {
                accumulator.push({
                    ...
                    "events": {
                        "available": getFormattedCounter(currentValue.data.results[0].events.available),
                    },
                    "stories": {
                        "available": getFormattedCounter(currentValue.data.results[0].stories.available),
                    },
                    "comics": {
                        "available": getFormattedCounter(currentValue.data.results[0].comics.available),
                    },
                    "series": {
                        "available": getFormattedCounter(currentValue.data.results[0].series.available),
                    }
                });
            }
        ...
        await useStorage('redis').setItem(cacheKeyMarvel, {...data, redis: "true"}, {
